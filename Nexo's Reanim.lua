...
pcall(function()
v.Velocity = Vector3.new(v.Velocity.X,30,v.Velocity.Z)
...
reanim.Parent = fl
reanim.HumanoidRootPart.CFrame = char.HumanoidRootPart.CFrame*CFrame.new(0,12,0)
...
part.AlignPosition.MaxForce = 9e4
...
function Pos(part,parent,p)return --coroutine.resume(coroutine.create(function()
--while task.wait() and part do part.Position=parent.Position end
--end))
end
function Pos1(part, parent, p)
Instance.new("Attachment",part)
Instance.new("AlignPosition",part)
Instance.new("Attachment",parent)
part.Attachment.Name = part.Name
...
Pos(char['HumanoidRootPart'],reanim['Torso'],Vector3.new(0,0,5))
...
fling_v=9e9*2
fling_val=Vector3.new(fling_v,fling_v,fling_v)--Vector3.new(9e9,9e9,9e9)
function flinger(p)
f=Instance.new('BodyAngularVelocity',p)
f.AngularVelocity = fling_val
f.MaxTorque= fling_val
end
...
if _G.amogumogu then return end
if click == true then
bp.Position=m.Hit.p
pcall(function()
...
_BRUH=game.Players.LocalPlayer.Chatted:Connect(function(msg)
    if HumanoidDied==true then _BRUH:Disconnect() end
    if msg=="-dfl" or msg=="/e -dfl" then
        _G.amogumogu=true
        coroutine.resume(coroutine.create(function()while _G.amogumogu do task.wait()
            pcall(function()char:WaitForChild("HumanoidRootPart").Position=Vector3.new(0,-5,0)end)
        end end))
    elseif msg=="-efl" or msg=="/e -efl" then
        _G.amogumogu=false
        wait()
    end
end)
