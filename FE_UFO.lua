...
FE UFO script edited by Siesgo#8832
Wrote my own sit script and made it so you can sit on the UFO lol

required hat: roblox.com/catalog/8151404994 (requires amazon prime to redeem)
]]

local INFORMATION_p=[[
Its recommended to use the loadstring to get the latest updates.
loadsting(game:HttpGet("https://raw.githubusercontent.com/RealSiesgo/Public_Scripts/main/FE_UFO.lua"))()
]]
...
sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",9e9)--sethiddenproperty(game.Players.LocalPlayer,"SimulationRadius",999999999)
...
local UFO = char:FindFirstChild("UFO");assert(UFO,"You need the ufo hat for this script to run!")
local fly = false
local campos = CFrame.new(0,0,0)
local cons = {}
local ti = table.insert

local TOGGLETRANSPARENCY=function(num)
    for i,v in next, char:GetChildren() do
        ...
        elseif v:IsA("Accessory") and v.Name~=tostring(UFO) then v=v.Handle
            if num==1 then
                v.Transparency=.9
            elseif num==2 then
                v.Transparency=0
...
ti(cons, game:GetService("RunService").RenderStepped:connect(function()
	...
	if fly == true then
	    h_j=char:WaitForChild("Humanoid",2)
	    if h_j and h_j.Sit==true then TOGGLETRANSPARENCY(1) end
		if wd == true then
...
function cpos()
    head=plr.Character:WaitForChild("Head",15)
    if not head then plr.CharacterAdded:Wait()
        head=plr.Character:WaitForChild("Head",5)
    end
    camera.CameraSubject=plr.Character:WaitForChild("Humanoid",5)
    camera.CoordinateFrame=head.CFrame
end

ti(cons,plr.CharacterAdded:Connect(function()
...
DRONE=UFO.Parent.Drone
DRONE.Seat.Orientation=Vector3.new(DRONE.Seat.Orientation.X,-90,DRONE.Seat.Orientation.Z)

lp=game.Players.LocalPlayer

SEAT=lp.Character.Drone.Seat

humanoid=lp.Character:FindFirstChildWhichIsA("Humanoid")

SIT_debounce=false
local CONNECTION1
local CONNECTION2
local SITCONNECTION
CONNECTION1=SEAT.Touched:Connect(function(part)
	if part.Parent.Name==lp.Name and part.Parent:FindFirstChildWhichIsA("Humanoid") and SIT_debounce==false and not humanoid.Sit then
		humanoid.Sit=true
		SITCONNECTION=SEAT:GetPropertyChangedSignal("CFrame"):Connect(function()
			if SIT_debounce==true then SITCONNECTION:Disconnect() end
		    humanoid.Sit=true
			lp.Character.HumanoidRootPart.CFrame=SEAT.CFrame+Vector3.new(0,1.5,0)
		end)
		coroutine.wrap(function()
			while SITCONNECTION.Connected do task.wait()
				if SIT_debounce==true then break end
				lp.Character.HumanoidRootPart.CFrame=SEAT.CFrame+Vector3.new(0,1.5,0)
				task.spawn(function()humanoid.Sit=true end)
			end
		end)()
		...
		JUMPCONNECTION=humanoid:GetPropertyChangedSignal("Jump"):Connect(function()
			...
			coroutine.wrap(function()SIT_debounce=true wait(2)SIT_debounce=false end)()
			JUMPCONNECTION:Disconnect()
		end)
	end
end)
function connectionz()
    if CONNECTION1 and CONNECTION1.Connected then CONNECTION1:Disconnect() end
    if CONNECTION2 and CONNECTION2.Connected then CONNECTION2:Disconnect() end
    if SITCONNECTION and SITCONNECTION.Connected then SITCONNECTION:Disconnect() end
end
coroutine.wrap(function()
    humanoid.Died:Wait() connectionz()
end)()
coroutine.wrap(function()
    lp.CharacterAdded:Wait() connectionz()
end)()
